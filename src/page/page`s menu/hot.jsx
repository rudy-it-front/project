...
import hots from '../../masiv/hotData.jsx';
...
function HotCard({ hot }) {
    ...
    const handleAddToCart = () => {
        ...
        addToCart(hot, selectedToppings, 'hot');
        setAddedToCart(true);
        setTimeout(() => {
    ...
    return (
        <Col xs="auto" className="d-flex mx-auto">
            <Card style={{ width: '25rem', height: '42rem' }} className="mx-2 my-2 my-card">
                ...
                <Card.Header>
                    <Card.Title>
                        {hot.name} ({hot.weight}г)
                    </Card.Title>
                </Card.Header>
                <Card.Text style={{ overflowWrap: 'break-word',
                    padding: '10px',
                    height:"100px" }}>
                    {formatDescription(hot.description).split('\n').map((line, index) => (
        ...
        <>
            ...
            <Container className="mt-2">
                <Row xs={1} md={2} lg={3} className="g-2">
                    {hots.map((hot) => (
                        <HotCard key={hot.id} hot={hot} addToCart={addToCart}/>
                    ))}
                </Row>
            </Container>
        </>
