...
import salads from '../../masiv/saladData.jsx';
...
function SaladCard({ salad }) {
    ...
    const handleAddToCart = () => {
        ...
        addToCart(salad, selectedToppings, 'salad');
        setAddedToCart(true);
        setTimeout(() => {
    ...
    return (
        <Col xs="auto" className="d-flex mx-auto">
            <Card style={{ width: '25rem', height: '42rem' }} className="mx-2 my-2 my-card">
                ...
                <Card.Header>
                    <Card.Title>
                        {salad.name} ({salad.weight}г)
                    </Card.Title>
                </Card.Header>
                <Card.Text style={{ overflowWrap: 'break-word',
                    padding: '10px',
                    height:"100px" }}>
                    {formatDescription(salad.description).split('\n').map((line, index) => (
        ...
        <>
            ...
            <Container className="mt-2">
                <Row xs={1} md={2} lg={3} className="g-2">
                    {salads.map((salad) => (
                        <SaladCard key={salad.id} salad={salad} addToCart={addToCart}/>
                    ))}
                </Row>
            </Container>
        </>
