...
import deserts from '../../masiv/desertData.jsx';
...
function DesertCard({ desert }) {
    ...
    const handleAddToCart = () => {
        ...
        addToCart(desert, selectedToppings, 'desert');
        setAddedToCart(true);
        setTimeout(() => {
    ...
    return (
        <Col xs="auto" className="d-flex mx-auto">
            <Card style={{ width: '25rem', height: '42rem' }} className="mx-2 my-2 my-card">
                ...
                <Card.Header>
                    <Card.Title>
                        {desert.name} ({desert.weight}г)
                    </Card.Title>
                </Card.Header>
                <Card.Text style={{ overflowWrap: 'break-word',
                    padding: '10px',
                    height:"100px" }}>
                    {formatDescription(desert.description).split('\n').map((line, index) => (
        ...
        <>
            ...
            <Container className="mt-2">
                <Row xs={1} md={2} lg={3} className="g-2">
                    {deserts.map((desert) => (
                        <DesertCard key={desert.id} desert={desert} addToCart={addToCart}/>
                    ))}
                </Row>
            </Container>
        </>
