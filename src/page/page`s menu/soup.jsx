...
import soups from '../../masiv/soupData.jsx';
import {useCart} from "../CartContext.jsx";
import PropTypes from "prop-types";
import {useState} from "react";

function SoupCard({ soup }) {
    ...
    const handleAddToCart = () => {
        ...
        addToCart(soup, selectedToppings, 'soup');
        setAddedToCart(true);
        setTimeout(() => {
    ...
    return (
        <Col xs="auto" className="d-flex mx-auto">
            <Card style={{ width: '25rem', height: '42rem' }} className="mx-2 my-2 my-card">
                ...
                <Card.Header>
                    <Card.Title>
                        {soup.name} ({soup.weight}г)
                    </Card.Title>
                </Card.Header>
                <Card.Text style={{ overflowWrap: 'break-word',
                    padding: '10px',
                    height:"100px" }}>
                    {formatDescription(soup.description).split('\n').map((line, index) => (
        ...
        <>
            ...
            <Container className="mt-2">
                <Row xs={1} md={2} lg={3} className="g-2">
                    {soups.map((soup) => (
                        <SoupCard key={soup.id} soup={soup} addToCart={addToCart}/>
                    ))}
                </Row>
            </Container>
        </>
